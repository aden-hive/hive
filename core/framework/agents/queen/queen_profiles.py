...
stored in ~/.hive/queens/{queen_id}/profile.yaml. Profiles are initialized
with rich defaults and can be edited via the API.
...
QUEENS_DIR = Path.home() / ".hive" / "queens"
...
def ensure_default_queens() -> None:
    ...
    for queen_id, profile in DEFAULT_QUEENS.items():
        profile_dir = QUEENS_DIR / queen_id / "agent"
...
def list_queens() -> list[dict[str, str]]:
    ...
    if not QUEENS_DIR.is_dir():
        return results
    for profile_path in sorted(QUEENS_DIR.glob("*/agent/profile.yaml")):
        queen_id = profile_path.parent.parent.name
        try:
...
def load_queen_profile(queen_id: str) -> dict[str, Any]:
    ...
    """
    profile_path = QUEENS_DIR / queen_id / "agent" / "profile.yaml"
