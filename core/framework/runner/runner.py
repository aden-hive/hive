    ...
    def _setup(self, event_bus=None) -> None:
        ...
        if self.mock_mode:
            ...
            if use_claude_code:
                ...
                if not api_key:
                    logger.warning("Codex subscription configured but no token found.")
                    logger.warning("Run 'codex' to authenticate, then try again.")
            elif use_kimi_code:
                # Get API key from Kimi Code CLI config (~/.kimi/config.toml)
                api_key = get_kimi_code_token()
                if not api_key:
                    logger.warning("Kimi Code subscription configured but no key found.")
                    logger.warning("Run 'kimi /login' to authenticate, then try again.")
                ...
                if self._is_local_model(self.model):
                    ...
                    if api_key_env and os.environ.get(api_key_env):
                        ...
                        if api_key:
                            ...
                            logger.warning(
                                "%s not set. LLM calls will fail.", api_key_env
                            )
                            logger.warning(
                                "Set it with: export %s=your-api-key", api_key_env
                            )
