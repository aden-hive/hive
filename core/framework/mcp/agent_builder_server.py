...
# =============================================================================
# BASIC TEST EXECUTION HELPERS (Provider-agnostic)
# =============================================================================

@mcp.tool()
def run_all_tests(agent_path: Annotated[str, "Path to agent export folder"]):
  """
  Simple wrapper to run all pytest tests inside agent folder.
  """
  import subprocess
...
@mcp.tool()
def list_test_files(agent_path: Annotated[str, "Path to agent export folder"]):
  """
  Lists all test_*.py files under agent tests directory.
  """
  ...
  return {
    "tests": [str(p) for p in tests_dir.glob("test_*.py")]
  }
...
- check: string (optional, how to validate: "llm_judge", expression, or function name)"""],
    ...
    # -------------------------------------------------
    # LLM-BASED TEST GENERATION REMOVED (Option A)
    # MCP ONLY PROVIDES GUIDELINES
    # -------------------------------------------------
    try:
