...
import { ChevronLeft, ChevronRight, Clock, Bot, Loader2, MoreHorizontal, Pencil, Trash2, Check, X, Plus } from "lucide-react";
...
interface HistorySidebarProps {
  ...
  /** When provided, a "New Chat" button is shown at the top of the sidebar. */
  onNewChat?: () => void;
...
export default function HistorySidebar({ onOpen, onNewChat, openSessionIds = [], activeSessionId, activeHistorySourceId, refreshKey }: HistorySidebarProps) {
    ...
    >
      ...
      {!collapsed && onNewChat && (
        <div className="px-2 py-2 border-b border-border/20">
          <button
            onClick={onNewChat}
            className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-[11px] font-medium text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors"
          >
      ...
      {collapsed && (
        <div className="flex-1 overflow-y-auto min-h-0 flex flex-col items-center py-2 gap-0.5">
          ...
          {onNewChat && (
            <button
              onClick={onNewChat}
              className="w-7 h-7 rounded-md flex items-center justify-center text-muted-foreground/40 hover:text-foreground hover:bg-muted/50 transition-colors"
              title="New Chat"
            >
