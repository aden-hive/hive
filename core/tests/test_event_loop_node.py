...
class TestClientFacingBlocking:
    ...
    @pytest.mark.asyncio
    async def test_reused_node_resets_shutdown_state(self, runtime, memory):
        """A reused node instance should not keep shutdown state between executions."""
        spec = NodeSpec(
            id="queen",
            name="Queen",
            description="stateful monitoring interface",
            node_type="event_loop",
            output_keys=["result"],
            client_facing=True,
        )
        ...
        first_llm = MockStreamingLLM(
            scenarios=[tool_call_scenario("ask_user", {"question": "first"}, tool_use_id="ask_1")]
        )
        first_ctx = build_ctx(runtime, spec, memory, first_llm)

        async def first_shutdown():
            await asyncio.sleep(0.05)
            node.signal_shutdown()

        first_shutdown_task = asyncio.create_task(first_shutdown())
        first_result = await node.execute(first_ctx)
        await first_shutdown_task
        assert first_result.success is True
        assert first_llm._call_index >= 1
        assert first_result.output == {}

        second_llm = MockStreamingLLM(
            scenarios=[
                tool_call_scenario("ask_user", {"question": "next"}, tool_use_id="ask_2"),
                tool_call_scenario(
                    "set_output",
                    {"key": "result", "value": "ok"},
                    tool_use_id="set_1",
        ...
        second_ctx = build_ctx(runtime, spec, memory, second_llm)

        async def inject_response():
            await asyncio.sleep(0.05)
            await node.inject_event("continue")

        injector = asyncio.create_task(inject_response())
        second_result = await node.execute(second_ctx)
        await injector

        assert second_result.success is True
        assert second_result.output["result"] == "ok"
        assert second_llm._call_index >= 2
