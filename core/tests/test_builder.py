...
async def create_successful_run(runtime: Runtime, goal_id: str = "test_goal") -> str:
    """Helper to create a successful run with decisions."""
    run_id = await runtime.start_run(goal_id, f"Test goal: {goal_id}")
    ...
    await runtime.record_outcome(d1, success=True, result={"items": 3}, tokens_used=50)
    ...
    await runtime.record_outcome(d2, success=True, result={"processed": 3}, tokens_used=30)

    await runtime.end_run(success=True, narrative="Successfully processed data")
    return run_id


async def create_failed_run(runtime: Runtime, goal_id: str = "test_goal") -> str:
    ...
    await runtime.record_outcome(d1, success=True, result={"items": 0})
    ...
    await runtime.record_outcome(d2, success=False, error="No data to process")
    ...
    await runtime.end_run(success=False, narrative="Failed to process - no data")
    return run_id
...
class TestBuilderQueryBasics:
    ...
    async def test_get_run_summary(self, tmp_path: Path):
        """Test getting a run summary."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        run_id = await create_successful_run(runtime)
    ...
    async def test_get_full_run(self, tmp_path: Path):
    ...
    async def test_list_runs_for_goal(self, tmp_path: Path):
        """Test listing all runs for a goal."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        await create_successful_run(runtime, "goal_a")
        await create_successful_run(runtime, "goal_a")
        await create_successful_run(runtime, "goal_b")
    ...
    async def test_get_recent_failures(self, tmp_path: Path):
        """Test getting recent failed runs."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        await create_successful_run(runtime)
        await create_failed_run(runtime)
        await create_failed_run(runtime)
...
class TestFailureAnalysis:
    ...
    async def test_analyze_failure(self, tmp_path: Path):
        """Test analyzing why a run failed."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        run_id = await create_failed_run(runtime)
    ...
    async def test_analyze_failure_returns_none_for_success(self, tmp_path: Path):
    ...
    async def test_failure_analysis_has_suggestions(self, tmp_path: Path):
    ...
    async def test_get_decision_trace(self, tmp_path: Path):
...
class TestPatternAnalysis:
    ...
    async def test_find_patterns_basic(self, tmp_path: Path):
        """Test basic pattern finding."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        await create_successful_run(runtime, "goal_x")
        await create_successful_run(runtime, "goal_x")
        await create_failed_run(runtime, "goal_x")
    ...
    async def test_find_patterns_common_failures(self, tmp_path: Path):
        """Test finding common failures."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        # Create multiple runs with the same failure
        for _ in range(3):
            await create_failed_run(runtime, "failing_goal")
    ...
    async def test_find_patterns_problematic_nodes(self, tmp_path: Path):
        """Test finding problematic nodes."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        # Create runs where process-node always fails
        for _ in range(3):
            await create_failed_run(runtime, "node_test")
    ...
    async def test_compare_runs(self, tmp_path: Path):
        """Test comparing two runs."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        run1 = await create_successful_run(runtime)
        run2 = await create_failed_run(runtime)
...
class TestImprovementSuggestions:
    ...
    async def test_suggest_improvements(self, tmp_path: Path):
        """Test generating improvement suggestions."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        # Create runs with failures to trigger suggestions
        for _ in range(3):
            await create_failed_run(runtime, "improve_goal")
    ...
    async def test_suggest_improvements_for_low_success_rate(self, tmp_path: Path):
        """Should suggest architecture review for low success rate."""
        runtime = Runtime(tmp_path)
        await runtime.initialize()
        # 4 failures, 1 success = 20% success rate
        for _ in range(4):
            await create_failed_run(runtime, "low_success")
        await create_successful_run(runtime, "low_success")
    ...
    async def test_get_node_performance(self, tmp_path: Path):
...
class TestBuilderWorkflow:
    ...
    async def test_builder_investigation_workflow(self, tmp_path: Path):
        ...
        for _ in range(2):
            await create_successful_run(runtime, "customer_goal")
        for _ in range(2):
            await create_failed_run(runtime, "customer_goal")
