...
export HIVE_AGENT_STORAGE_ROOT="/tmp/hive_agents_demo_workflows"

WATCH_API="/tmp/watched_requests_workflows_api"
WATCH_CSV="/tmp/watched_requests_workflows_csv"
LOG_API="/tmp/procurement_workflows_api.log"
LOG_CSV="/tmp/procurement_workflows_csv.log"
FIFO_API="/tmp/procurement_workflows_api.fifo"
FIFO_CSV="/tmp/procurement_workflows_csv.fifo"
...
pause() {
  local seconds="$1"
  echo "[demo-workflows] sleeping ${seconds}s..."
  sleep "$seconds"
}
...
start_monitor() {
  ...
  uv run python -m examples.templates.procurement_approval_agent monitor \
    --watch-dir "$watch_dir" \
    --poll-interval 1.0 \
    --mock \
    --interactive \
  ...
  MON_PID=$!
  echo "[demo-workflows] monitor pid: $MON_PID"
}
...
stop_monitor() {
  ...
  if [[ -n "${MON_PID:-}" ]] && kill -0 "$MON_PID" 2>/dev/null; then
    ...
    echo "[demo-workflows] monitor stopped (pid: $MON_PID)"
  fi
...
step "Path A: API route with interactive checkpoints"
uv run python -m examples.templates.procurement_approval_agent reset-setup
...
echo "[demo-workflows] dropped request: $WATCH_API/request_api.json"
feed_answers "$FIFO_API" "yes" "yes" "yes"

if wait_for_file "$API_RESULT" 45; then
  echo "[demo-workflows] API result file detected"
else
  echo "[demo-workflows] ERROR: API result file not detected within timeout"
fi
pause 2
stop_monitor

step "Path B: CSV route with interactive checkpoints"
uv run python -m examples.templates.procurement_approval_agent reset-setup
...
echo "[demo-workflows] dropped request: $WATCH_CSV/request_csv.json"
feed_answers "$FIFO_CSV" "yes" "no" "yes"

if wait_for_file "$CSV_RESULT" 45; then
  echo "[demo-workflows] CSV result file detected"
else
  echo "[demo-workflows] ERROR: CSV result file not detected within timeout"
fi
pause 2
stop_monitor

step "Monitor output logs"
echo "[demo-workflows] API log"
sed \
  -e '/paused_at=None is not a valid node, falling back to entry point/d' \
  -e '/end_run called but no run for execution/d' \
  "$LOG_API"
echo "[demo-workflows] CSV log"
sed \
  -e '/paused_at=None is not a valid node, falling back to entry point/d' \
  -e '/end_run called but no run for execution/d' \
  "$LOG_CSV"
...
step "Validation"
uv run python -m examples.templates.procurement_approval_agent validate
uv run python -m examples.templates.procurement_approval_agent info

echo
echo "[demo-workflows] Demo complete."
