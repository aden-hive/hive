...
@click.option(
    ...
    default=False,
    help="Prompt for yes/no workflow checkpoints.",
)
@click.option(
    "--process/--skip-process",
    default=True,
    help="Default request processing decision.",
)
@click.option(
    "--sync-confirm/--sync-cancel",
    default=True,
    help="Default final sync/export decision.",
)
@click.option(
    "--sync-method",
...
def run(
    ...
    if interactive:
        ...
        if process:
            ...
            if qb_credential_ref:
                default_has_qb = has_quickbooks_api_credentials(
                    credential_ref=qb_credential_ref
                )
...
def tui() -> None:
    ...
    click.echo(
        "Example: ./hive run examples/templates/procurement_approval_agent --tui"
    )
...
@click.option(
    "--auto-open-csv",
    is_flag=True,
    help="Reveal CSV export in file manager on fallback.",
)
@click.option(
    "--notify/--no-notify",
    default=True,
    help="Send Slack/SMTP notifications if configured.",
)
...
@click.option(
    "--log-file",
    default="/tmp/procurement_approval_agent_monitor.log",
    show_default=True,
)
def monitor(
    watch_dir,
    ...
    click.echo(
        f"Monitoring {watch_path} for request files (*.json). Press Ctrl+C to stop."
    )
    try:
...
@click.option(
    "--label", default="com.hive.procurement-approval-agent", show_default=True
)
@click.option(
    "--destination",
...
@click.option(
    "--log-file",
    default="/tmp/procurement_approval_agent_launchd.log",
    show_default=True,
)
