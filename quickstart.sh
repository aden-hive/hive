...
if [ "$OLLAMA_DETECTED" = true ]; then
    echo -e "  ${CYAN}10)${NC} Local (Ollama) - No API key needed  ${GREEN}(ollama detected)${NC}"
else
    echo -e "  ${CYAN}10)${NC} Local (Ollama) - No API key needed"
fi
...
if [ -n "$SELECTED_PROVIDER_ID" ]; then
    ...
    if [ "$SUBSCRIPTION_MODE" = "claude_code" ]; then
        ...
        save_configuration "ollama" "" "$SELECTED_MODEL" "$SELECTED_MAX_TOKENS" "" "http://localhost:11434" > /dev/null
    else
